  ...
  return (
    <header className="p-4 md:p-5 shadow-md bg-white sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <Link
          to="/"
          className="text-2xl md:text-4xl font-bold text-[oklch(0.41_0.11_231.91)]"
        >
          Jenova
        </Link>
          ...
          <div className="mt-4 px-3 space-y-3">
            {isAuthenticated ? (
              <>
                <Link
                  to="/user/profile"
                  className="flex items-center w-full font-bold text-gray-700 hover:text-blue-500 hover:bg-gray-50 rounded-lg transition-colors"
                  onClick={() => setIsMenuOpen(false)}
                >
                  Profile
                </Link>
